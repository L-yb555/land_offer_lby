...
自动驾驶数据闭环、感知数据挖掘、VLM 刷库、结构化打标、Qwen2-VL、One-Caption-Model、Grounding DINO / GDINO、开放词汇目标检测、Custom-VLM / Public-VLM、Prompt 设计、JSON 标签输出、tagger 解析、RabbitMQ 异步推理、LLaMA-Factory、xtuner、SFT、LoRA / QLoRA、Precision / Recall / F1-score、混淆矩阵、Corner Case 挖掘、标签库建设、以图搜图 2.0、相似性召回与正负例精排、闸机杆五分类 SFT。
...
6. ~~**`one_caption_model` 基座口径冲突**~~ —— **已确认并修正**：`one_caption_model` 是 **Qwen2-VL** 系列（Qwen2-VL-2B 用于驾驶场景分类；Qwen2-VL vLLM 用于工况+车位 7 维打标）；本文 2.1 的 Qwen3-VL-8B / Qwen3.5-9B SFT 主线为闸机杆五分类任务，两者是不同组件，勿混淆。
7. **撤稿说明（2026-09-23）**：旧口径「泊车挡轮杆识别、基于 GRPO 的三元组策略优化」与「Corner Case 数据合成（规则渲染 + 生成模型、5K 样本）」叙述已从全链路撤掉（`../baseline.md`、各简历 md 与 `template/*.tex`）；对外口径以本文 **闸机杆五分类 SFT 主线（2.1）** 与 **以图搜图 2.0（十六）** 为准。



## 十六、感知数据挖掘与以图搜图 2.0（2026-09-23 补充收录）

### 16.1 感知数据挖掘

- **背景**：面向感知模型在训练、评测与迭代过程中的多样化数据需求，针对复杂场景、长尾场景及 Corner Case 样本获取难、覆盖不足的问题，构建高效、可扩展的场景挖掘与标签交付能力，支撑感知模型持续优化。
- **主要工作**：
  - 面向感知模型训练、评测、地图团队迭代需求，批量交付高质量场景样本与感知标签，确保准确率超过 80%；
  - 基于 Qwen3-VL、以图搜图、多源 PB 信号等多模态挖掘方案，形成可复用的场景挖掘、复核与标签交付流程。
- **主要技术**：Python、PyTorch、protobuf、C++、pandas / NumPy。

### 16.2 以图搜图 2.0 方案设计与落地

- **背景**：传统感知数据挖掘通常依赖模型实时推理，存在成本高、启动慢等问题。为提升长尾场景挖掘效率，设计并落地基于视觉相似性的「以图搜图 2.0」方案，实现低成本、低依赖、快速启动的感知数据建设能力。
- **主要工作与结果**：
  - 主导设计并落地「以图搜图 2.0」，基于相似性、正负例精排进行场景召回与标签生产，不依赖检测、分割等额外模型；
  - 累计交付 **50+ 感知标签**，覆盖蹲着的行人、地面积水中的车辆倒影等高价值 Corner Case 场景；
  - 单标签样本规模通常 **>1.5 万**，整体**标签准确率超过 80%**，在保证质量的同时实现稳定交付；
  - 人效稳定在 **2 个标签 / 人 / 天**，显著提升感知标签生产效率，支持中小团队或早期阶段快速开展数据建设与模型迭代。