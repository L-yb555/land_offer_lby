...
\begin{itemize}
    ...
    \item \textbf{AI 与数据}：PyTorch、Ultralytics YOLO、Qwen3-VL、LLaMA-Factory、GRPO、TensorRT、ONNX、NVIDIA Jetson AGX Orin、4 机 8 卡 H20 分布式训练。
...
\begin{itemize}
    \item 融合车载多路传感器与回灌数据设计规则化挖掘算子，红绿灯自车转向 Tagger 在 3W 条路口数据中筛选 3K 条 CLIP，\hl{验收 90\%、挖掘召回提升 10\%}；针对实车难覆盖 Corner Case 合成 \hl{5K 训练样本}。
    \item 负责泊车障碍物识别 VLM 优化：筛选 1.8 万张挡轮杆图像完成数据清洗、标注校验与 Prompt 迭代，基于 Qwen3-VL-8B + LLaMA-Factory \hl{在 4 机 8 卡 H20 完成全参数 SFT}，设计基于 GRPO 的三元组策略优化，完成模型二次精标业务交付。
\end{itemize}
...
\begin{itemize}
    \item \hl{已中（JCR 一区 / 中科院二区）}：Liang B., Li N., Li G., et al. Sensing Technologies for Hand Gesture Recognition in Human-Robot Interaction: A Review[J]. IEEE Sensors Journal, 2025, 26(2): 1501-1519. \textbf{综述}：梳理数据手套、视觉与肌电等手势感知路线，对比精度、侵入性与成本取舍。
