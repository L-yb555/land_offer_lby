...
\begin{itemize}
    ...
    \item 负责感知细粒度识别 VLM 后训练优化，筛选清洗闸机杆五分类图像并完成数据清洗、标注校验与 Prompt 迭代，基于 Qwen3-VL-8B + LLaMA-Factory 完成 13 轮 SFT 实验迭代与 H20 8GPU 集群全参数 SFT，\hl{测试集准确率 0.8984 创历史新高}，沉淀可复用 SFT 实验结论。
    \item 主导设计并落地“以图搜图 2.0”（相似性 + 正负例精排做场景召回与标签生产，不依赖检测/分割等额外模型），累计交付 \hl{50+ 感知标签、整体标签准确率超 80\%}。
\end{itemize}
