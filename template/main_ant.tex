...
{\scriptsize\color{WHU_Blue} 感知数据闭环 / VLM 后训练 / 长尾场景挖掘与样本合成}
\begin{itemize}
    ...
    \item 融合车载多路传感器与回灌数据设计规则化挖掘算子，红绿灯自车转向 Tagger 在 3W 条路口数据中筛选 3K 条 CLIP，\hl{标签验收准确率达 90\%、挖掘召回提升 10\%}；针对实车难覆盖的废弃地面箭头、引导线等 Corner Case 设计“规则渲染 + 生成模型”合成 pipeline，\hl{生成 5K 训练样本}，支撑挖掘模型冷启动迭代。
    \item 负责泊车障碍物识别 VLM 优化：筛选 1.8 万张挡轮杆图像完成数据清洗、标注校验与 Prompt 迭代，基于 Qwen3-VL-8B + LLaMA-Factory \hl{在 4 机 8 卡 H20 完成全参数 SFT}；针对类别误判、高度不足等验收问题设计基于 GRPO 的三元组策略优化方法，完成模型二次精标业务交付。
\end{itemize}
